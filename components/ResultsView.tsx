    ...
    <div className="max-w-6xl mx-auto px-4 py-8 animate-in fade-in duration-300">
      <div className="mb-6 flex items-center space-x-4">
        <div>
            ...
            <div className="h-1 w-12 bg-liberia-red rounded-full mt-2"></div>
        </div>
        {/* Subtle streaming indicator */}
        {!result.text && <Loader2 className="w-5 h-5 text-liberia-blue animate-spin" />}
      </div>
        ...
        <div className="md:col-span-2">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 md:p-10">
                    <div className="flex justify-between items-center mb-8">
                        <div className="flex items-center space-x-2 bg-blue-50 px-3 py-1 rounded-full border border-blue-100">
                            <Sparkles className="w-3.5 h-3.5 text-liberia-blue" />
                    ...
                    <div className="prose prose-blue max-w-none text-gray-700 leading-relaxed font-sans text-lg">
                        <ReactMarkdown
                          components={{
                        ...
                        {!result.sources.length && result.text && <span className="inline-block w-2 h-5 ml-1 bg-liberia-blue animate-pulse align-middle"></span>}
                    </div>
                </div>
                    ...
                    <div className="bg-gray-50/50 p-6 md:p-10 border-t border-gray-100">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {result.sources.map((source, idx) => (
                                <a key={idx} href={source.uri} target="_blank" rel="noreferrer" className="flex items-center p-3 bg-white rounded-xl border border-gray-100 hover:border-liberia-blue transition-all group shadow-sm">
                                    <ExternalLink className="w-4 h-4 text-gray-300 group-hover:text-liberia-blue mr-3 flex-shrink-0" />
                ...
                <div className="px-6 py-4 border-t border-gray-50 flex justify-end space-x-6">
                    <button onClick={handleShare} className="text-xs font-bold text-gray-400 hover:text-liberia-blue flex items-center">
                        <Share2 className="w-4 h-4 mr-1.5" /> Share
                    </button>
                    <button onClick={handleShare} className="text-xs font-bold text-gray-400 hover:text-liberia-blue flex items-center">
                        <LinkIcon className="w-4 h-4 mr-1.5" /> {showCopied ? 'Copied!' : 'Copy Link'}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div className="md:col-span-1 space-y-6">
            <div className="bg-gradient-to-br from-liberia-blue to-blue-900 rounded-2xl p-6 text-white shadow-lg shadow-blue-900/20">
                <div className="flex items-center mb-4">
                    <Sparkles className="w-5 h-5 text-liberia-gold mr-2" />
                    <h4 className="font-bold text-sm tracking-wide">Real-time Grounding</h4>
                </div>
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                ...
                <button onClick={onBusinessClick} className="w-full py-2 bg-slate-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-700 hover:bg-gray-100">Access API Console</button>
            </div>
        </div>
      </div>
    </div>
